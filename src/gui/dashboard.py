...
class DP100Dashboard:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Create Dash app
        self.app = dash.Dash(
            __name__, external_stylesheets=[dbc.themes.BOOTSTRAP], title="DP100 Monitor"
        )
    ...
    def _device_monitor_loop(self) -> None:
        ...
        while self.running:
            if not self.data_collector.is_running():
                self.logger.debug(
                    "Data collector is not running, attempting to start..."
                )
                try:
    ...
    def _setup_callbacks(self) -> None:
            ...
            if triggered_id == "start-button" and start_clicks:
                ...
                if success:
                    ...
                    new_alert = create_alert(
                        "Failed to start data collection", "danger"
                    )
                ...
                triggered_id == "set-voltage-button"
                and voltage_clicks
                ...
                if success:
                    new_alert = create_alert(
                        f"Voltage set to {voltage_value}V", "success"
                    )
                else:
                    new_alert = create_alert("Failed to set voltage", "danger")

            elif (
                triggered_id == "set-current-button"
                and current_clicks
                ...
                if success:
                    new_alert = create_alert(
                        f"Current limit set to {current_value}A", "success"
                    )
                else:
                ...
                if success:
                    ...
                    new_alert = create_alert(
                        f"Failed to {state.split()[0]} output", "danger"
                    )
        ...
        def update_display(n_intervals, reset_clicks):
            ...
            for sample in samples:
                delta_t = (
                    sample.timestamp - self.last_integration_timestamp
                ).total_seconds()
                if delta_t > 0:
                    # mWh = (W * 1000) * (s / 3600)
            ...
            voltage_text = (
                f"{latest['voltage']:.2f}" if latest["voltage"] is not None else "0.00"
            )
            current_text = (
                f"{latest['current']:.3f}" if latest["current"] is not None else "0.000"
            )
            power_text = (
                f"{latest['power']:.2f}" if latest["power"] is not None else "0.00"
            )

            # 4. Get stats for display
            stats = self.data_collector.get_statistics()
            collection_status_str = (
                "Saving to file" if self.collecting else "Not saving"
            )

            status_text = [
                html.P(f"Status: {collection_status_str}", className="mb-1"),
                html.P(
                    f"Rate: {stats.get('samples_per_second', 0):.1f} Hz",
                    className="mb-1",
                ),
            ]
            ...
            if self.data_collector.is_running():
                ...
                statistics_content = html.P(
                    "Device not connected.", className="text-muted"
                )
    ...
    def create_layout(self) -> html.Div:
        ...
        return dbc.Container(
            [
                ...
                dcc.Interval(
                    id="update-interval", interval=100, n_intervals=0  # 100ms default
                ),
            ],
            fluid=True,
        )
