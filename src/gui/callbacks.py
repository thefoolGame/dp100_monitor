...
class CallbackManager:
    ...
    def _register_control_callbacks(self) -> None:
        ...
        @self.dashboard.app.callback(
            ...
            [State("voltage-input", "value"), State("current-input", "value")],
        )
        def handle_device_controls(
            voltage_clicks, current_clicks, output_enabled, voltage_value, current_value
        ):
            ...
            if triggered_id == "set-voltage-button" and voltage_clicks:
                if voltage_value is not None:
                    ...
                    self.logger.info(
                        f"Voltage set to {voltage_value}V: {'success' if success else 'failed'}"
                    )
                ...
                if current_value is not None:
                    ...
                    self.logger.info(
                        f"Current set to {current_value}A: {'success' if success else 'failed'}"
                    )
                ...
                self.logger.info(
                    f"Output {state}: {'success' if success else 'failed'}"
                )
    ...
    def _register_settings_callbacks(self) -> None:
        ...
        @self.dashboard.app.callback(
            ...
            [Input("time-window-select", "value"), Input("decimation-select", "value")],
        )
        def update_plot_settings(window_seconds, decimation_factor):
            ...
            try:
                ...
                if decimation_factor:
                    self.dashboard.realtime_plot.set_decimation_factor(
                        decimation_factor
                    )
                    self.logger.debug(
                        f"Decimation factor updated to {decimation_factor}"
                    )
