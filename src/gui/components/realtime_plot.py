...
class RealtimePlot:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.max_points = int(
            self.plot_window_seconds * 50 / self.decimation_factor
        )  # Assume 50Hz max
        ...
        # Statistics
        self.stats = {"points_plotted": 0, "last_update": None, "update_rate": 0.0}

    def add_data_point(
        self, timestamp: datetime, voltage: float, current: float, power: float
    ) -> None:
        """
    ...
    def create_figure(self) -> go.Figure:
        ...
        if timestamps_list:
            ...
            fig.update_xaxes(range=[start_time, latest_time], row=3, col=1)
    ...
    def get_latest_values(self) -> Dict[str, Optional[float]]:
        ...
        if not self.timestamps:
            return {"voltage": None, "current": None, "power": None, "timestamp": None}
