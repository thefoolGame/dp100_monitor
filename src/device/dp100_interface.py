...
class DP100Interface:
    ...
    def get_full_status(self) -> Optional[DP100Status]:
        ...
        try:
            ...
            temperature = (
                ((response[15] << 8) | response[14]) / 10.0
                if len(response) > 15
                else 25.0
            )
    ...
    def _gen_set_payload(
        self, output: bool, vset: int, iset: int, ovp: int = 30500, ocp: int = 5050
    ) -> bytes:
    ...
    def set_voltage(self, voltage: float) -> bool:
        ...
        try:
            ...
            data = self._gen_set_payload(
                self.output_enabled, voltage_mv, self.current_current_set
            )
    ...
    def set_current(self, current: float) -> bool:
        ...
        try:
            ...
            data = self._gen_set_payload(
                self.output_enabled, self.current_voltage_set, current_ma
            )
    ...
    def set_output(self, enabled: bool) -> bool:
        ...
        try:
            ...
            data = self._gen_set_payload(
                enabled, self.current_voltage_set, self.current_current_set
            )
