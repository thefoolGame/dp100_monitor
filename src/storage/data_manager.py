...
class DataManager:
    ...
    def start_session(self, session_id: Optional[str] = None) -> SessionInfo:
        ...
        self.current_session.backup_path = str(self.exports_dir / f"session_{session_id}.csv")
        
        # Create HDF5 file
        try:
            self.hdf5_file = h5py.File(self.current_session.file_path, 'w')
            # Create datasets with chunking for efficient appending
            dataset_kwargs = {'chunks': (min(1000, self.buffer_size),), 'maxshape': (None,)}
            self.hdf5_file.create_dataset('timestamp', (0,), dtype='S26', **dataset_kwargs)
    ...
    def end_session(self) -> None:
        ...
        self.running = False
        if self.writer_thread: self.writer_thread.join(timeout=5.0)
        ...
        self.logger.info(f"Ended session: {self.current_session.session_id} ({self.current_session.sample_count} samples)")
    ...
    def _flush_buffer(self) -> None:
        ...
        if not self.write_buffer or not self.hdf5_file: return
        ...
        try:
            ...
            if self.current_session: self.current_session.sample_count += count
    ...
    def _export_csv(self) -> None:
        ...
        if not self.current_session or not self.current_session.backup_path: return
        
        try:
            with h5py.File(self.current_session.file_path, 'r') as f:
                df = pd.DataFrame({
                    'timestamp': pd.to_datetime([ts.decode('utf-8') for ts in f['timestamp'][:]]),
                    'voltage': f['voltage'][:],
                    'current': f['current'][:],
                    'power': f['power'][:]
                })
